...
async function verifyFace() {
  ...
  try {
    ...
    const res = await fetch(
      `${import.meta.env.VITE_API_URL}attendances/checkin/face`,
      {
        method: "POST",
        body: formData,
      },
...
<template>
  <div class="flex flex-col items-center gap-4">
    <Card
      class="bg-emerald-950/10 w-160 h-120 overflow-hidden p-0 relative flex items-center justify-center border-2 border-emerald-500 drop-shadow-xl drop-shadow-emerald-500/50 backdrop-blur-xs"
    >
      <CardContent>
        <div class="relative w-160 h-120">
          ...
          <!-- Status overlay -->
          <div
            class="absolute top-4 left-4 bg-emerald-500/80 px-4 py-2 rounded-lg font-semibold flex items-center gap-2"
          >
            <span class="text-emerald-900">{{ statusText }}</span>
            <span v-if="countdown > 0" class="text-yellow-400">{{
              countdown
            }}</span>
            <span v-if="statusText === 'Checked!'" class="text-emerald-900"
              >✓</span
            >
          </div>
        </div>
